...
> **版本**：v1.1 | **基线日期**：2026-08-05 | **适用范围**：agent-demo
...
| LLM 接入 | 同步对话模型 | ChatModel（doubao-seed-2.0 / deepseek-v4-flash） | ✅ |
| LLM 接入 | 流式对话模型 | StreamingChatModel（SSE） | ✅ |
| LLM 接入 | Embedding 模型 | 豆包 Embedding / text-embedding-v4（RAG 用） | ✅ |
| LLM 接入 | 场景路由 | chat/code/lite 按需切换 | ✅ |
| LLM 接入 | 多厂商注册表路由 | 能力矩阵 + 提供商策略 + 注册表，`llm.provider` 切换（CR-002） | ✅ |
| LLM 接入 | 视觉对话模型 | VisionChatModel（能力检测，未实现抛 UnsupportedCapabilityException，CR-002） | ✅ |
...
| LLM 接入 | 思考流式模型 | 继承 AbstractThinkingStreamingChatModel 基类，厂商子类仅实现差异化钩子（CR-001 新增，CR-002 重构） | ✅ |
...
| **LLM 接入** | `llm.registry` / `llm.provider` / `llm.config` / `llm.capability` / `llm.thinking` / `llm.exception` | ModelFactory、ArkLlmServiceProvider、BailianLlmServiceProvider、LlmProperties | - | 多 LLM 厂商统一接入（火山引擎/阿里百炼）与注册表路由（CR-002 重构） |
...
erDiagram
    ...
    ModelFactory ||--o{ LlmServiceProvider : "注册表路由（CR-002）"
    LlmServiceProvider ||--o{ ChatModel : "创建缓存（Provider 内部）"
    LlmServiceProvider ||--o{ StreamingChatModel : "创建缓存（Provider 内部）"
    LlmServiceProvider ||--o| EmbeddingModel : "创建单例（Provider 内部）"
...
| LLM 接入 | `llm.registry` / `llm.provider` / `llm.config` / `llm.capability` / `llm.thinking` / `llm.exception` | ModelFactory、LlmServiceProvider、ArkLlmServiceProvider、BailianLlmServiceProvider、AbstractThinkingStreamingChatModel、UnsupportedCapabilityException |
...
| 20 | BR-LLM-008 | LLM 提供商通过 `llm.provider` 配置项切换（`ark` / `bailian`），默认 `ark`（CR-002） | LLM 接入 | 强制 |
| 21 | BR-LLM-010 | 切换提供商后只校验当前提供商的 API Key，未激活的提供商不校验（CR-002） | LLM 接入 | 强制 |
| 22 | BR-LLM-014 | 新增 LLM 厂商时 `ModelFactory` 核心代码必须零修改，仅通过新增 `@Component` 实现类完成接入（CR-002） | LLM 接入 | 强制 |
| 23 | BR-LLM-015 | `ModelFactory` 中禁止出现 `if (provider == XXX)` 形式的厂商硬编码分支（CR-002） | LLM 接入 | 强制 |
| 24 | BR-LLM-016 | 思考流式模型厂商实现代码重复率必须 ≤ 30%，通过继承 `AbstractThinkingStreamingChatModel` 实现（CR-002） | LLM 接入 | 强制 |
| 25 | BR-LLM-017 | 厂商未实现的能力接口在运行时必须抛出 `UnsupportedCapabilityException`，禁止隐式失败（CR-002） | LLM 接入 | 强制 |
...
| 新增 Agent 实现 | - | **R/A** | - | I |
| 接入新 LLM 模型 | - | **R** | I | **A** |
| 接入新 LLM 厂商（CR-002） | - | **R/A** | - | I |
| 配置 API Key | - | C | - | **R/A** |
| 切换 LLM 场景路由 | - | **R** | I | **A** |
| 切换 LLM 提供商（CR-002） | - | C | I | **R/A** |
| 会话超时调优 | - | C | I | **R/A** |
| 安全约束调整 | - | **R** | I | **A** |
