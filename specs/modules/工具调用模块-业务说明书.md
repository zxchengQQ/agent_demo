...
- **触发场景**：运行时新增工具（如 MCP 加载的外部工具）。
- **操作步骤**：`ToolRegistry.register(tool)`。
- **系统行为**：校验工具类有 `@Tool` 注解，加入工具列表，立即可被 Agent 使用。
- **业务规则**：动态注册的工具与内置工具享有同等地位。
...
- **ToolRegistry**：工具注册中心，管理工具列表（CopyOnWriteArrayList），懒加载扫描，支持动态注册。
...
| `ToolRegistry.register(tool)` | 动态注册工具 | mcp 层（规划中） |
